...
//
// Found by rebuilding the replica-ui fixture (scripts/replica-ui.sh) — the one
...
// TestACommitClosesTheUnitItNAMES is field-test finding #50's other half.
//
...
// ---------- the fields that were declared and never read ----------

// TestThePhaseBriefingCarriesTheWhenCondition — `phases[].when`.
//
...
// TestTheGateBriefingCarriesTheCoverageFloors — `domains[].coverage`.
//
...
// TestOnExceedWarnSaysSoInsteadOfPassingInSilence — `budget.on_exceed: warn`.
//
...
// user declared which one they wanted: wherever batten softens an enforcement, a rule decides.
