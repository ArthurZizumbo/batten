...
//
// Two batten binaries routinely share ~/.batten/batten.db: the one the plugin installed and
// whatever is on your PATH. `batten doctor` warns when their versions disagree precisely because it
// is normal for them to. So a migration that DROPPED or RENAMED anything would leave the older
// binary reading a file the newer one had already changed — inside a hook, which fails open. The
// gate would go quiet, and nothing would say so.
//
// Add a column, add a table, never take one away. When something really must go, the removal is a
// separate release after nothing reads it — expand, migrate, then contract, with the contraction
// paid for by a version bump and not by a user's silent afternoon.
//
// TestEveryMigrationIsAdditive holds this, and it holds len(migrations) == schemaVersion too: the
// loop below indexes this slice by version, so a mismatch either skips a migration or panics with
// an index out of range, in a hook, where a panic is indistinguishable from silence.
