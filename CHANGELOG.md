...
First tag after the beta. Minor rather than patch because the build itself changed: what the
release compiles, and what the binary reports about itself, are both different now.

### Fixed

- **`-X main.version` never worked, and nothing said so.** `.goreleaser.yaml` has always passed
  `-X main.version={{.Version}}`, but `main.version` was initialised with a function call
  (`buildVersion("0.1.0")`), and the Go linker **silently ignores `-X`** unless the variable is
  declared uninitialised or with a constant. No error, no warning: every release reported whatever
  `debug.ReadBuildInfo` happened to embed, and `batten doctor` compared *that* against
  `plugin.json`. Now `version` is a plain `""` with the fallback moved into `init()`.
  `TestTheLinkerCanStampTheVersion` runs the linker to hold the line, because the defect is
  invisible to anything that only reads source. **[dogfood]**
- **The published binaries embedded the author's absolute source paths** —
  `C:/Users/<user>/…/cmd/batten/main.go`, in all six. `-s -w` does *not* remove them; only
  `-trimpath` does, so the release looked stripped and clean while it was not. The existing
  personal-path guards read *files* and could never see inside a compiled binary.
  `TestTheReleasedBinaryLeaksNoPersonalPaths` closes that gap.

### Added

- **A Windows VERSIONINFO resource in `batten.exe`.** CompanyName, ProductName, FileDescription,
  LegalCopyright and OriginalFilename now appear in Properties → Details; a Go binary ships with
  all of them empty. Generated by `scripts/gen-winres.{sh,ps1}` from `cmd/batten/versioninfo.json`.
  Measured cost: **1024 bytes** on a 15 MB binary.
  **This does not stop Windows Defender flagging the binary, and nothing claims it does** — see
  *Known gaps*. It removes one cheap negative signal; that is the whole claim.
- **`scripts/set-version.sh <version>`** — writes the version into both manifests, which are the
  only two places a human had to edit by hand. They cannot read an environment variable: Claude
  Code parses those files as literal JSON, with no interpolation step.

### Changed

- `scripts/build-plugin.{sh,ps1}` now build exactly what the release builds — `-trimpath`, the
  version resource, and the same `-X main.version`. A locally built binary is the answer given to
  anyone hit by the antivirus false positive, so it must not be a different artifact.

### Known gaps

- **The false positive is not fixed, and cannot be proven fixed.** Measured 2026-07-31 with engine
  `1.1.26060.3008` and definitions `1.455.440.0`: **0/10 detections on the old build and 0/10 on
  the new one**, with an EICAR control confirming the scanner was live on that path. The baseline
  did not fire either, so **the experiment has no discriminating power** and the pre-registered
  threshold (≤5/10) is unevaluable. What it does confirm, for the third independent time, is that
  the classifier is stochastic.
- **batten is still unsigned.** Authenticode is the only thing that changes the category of the
  problem, and it does not eliminate it either.
